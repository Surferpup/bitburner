    ...
    */
	...
	const prepareServers = async (servers) => {
		for (const serv of servers) {
			...
			ns.scp(viruses, servName); // copy viruses over
			if (!serv.isOwned) {
    ...
    const intelligenceBonus = (1 + (1 * Math.pow(1, 0.8)) / 600);
    ...
    function hackingChance(server, person) {
    ...
    function calculateHackingTime(server, person) {
        ...
        const hackingTime =
            ...
            (person.mults.hacking_speed * intelligenceBonus);
    ...
    function calculateWeakenTime(server, person) {
        const weakenTimeMultiplier = 4; // Relative to hacking time. 16/5 = 3.2
    ...
    const getNodeInfo = (node) => {
		...
		const hackChance = hackingChance(server, player);
	...
	function getDelayForActionSeq(seq, node) {
		...
		const hTime = calculateHackingTime(server, player);
        const wTime = hTime * 4
        const gTime = hTime * 3.2
        //const wTime = calculateWeakenTime(server, player);
		//const gTime = calculateGrowTime(server, player);
	...
	function getMaxThreads(node) {
		...
		var weakenThreads = weakenEffect > 0 ? Math.round(secToDecrease / weakenEffect) : 0;
		// Hack calculation
